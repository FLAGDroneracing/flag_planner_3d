...
int flag_A = 1;                              // 0: disable mission A debug       1: enable mission A debug
...
double LAMDA_WALL = 6.0;
double LAMDA_TUNNEL = 3.0;
double LAMDA_CIRCLR = 0.5;
double LAMDA_RING = 0.5;

double HEIGHT_WALL = 0.8;
double HEIGHT_TUNNEL = 1.75;
double HEIGHT_PLAN = 0.8;
double HEIGHT_CIRCLE = 1.5;
double HEIGHT_RING = 1.5;

double TRANS_WALL = 3.0;
double TRANS_TUNNEL = 3.0;
double TRANS_PLAN = 3.0;
...
void Set_YawSearch()
{
    if(rotate_direct == 1) 
    {
        yaw_search = yaw_search - 1.5;
        if(yaw_search < -180.0) {yaw_search = yaw_search + 360.0;}
    }
    else                   
    {
        yaw_search = yaw_search + 1.5;
    ...
    if( (yaw_search > 0.0) && (yaw_search < 120.0) ) {rotate_direct = 0;}
    if( (yaw_search < 0.0) && (yaw_search > -120.0) ) {rotate_direct = 1;}
}
...
*/
    ...
    pos_drone_now = Eigen::Vector3d(0.0, 0.0, 0.8);
...
//        if(cmdd!=4) local_pos_pub.publish(aim);
        ...
        if(cmdd == 7)
        {
            ...
            {    
                ...
                if(flag_wall == 0)
                {
                    /*----------fly towards 1m before wall----------*/
                    if(pos_drone_temp[0] < TRANS_WALL)
                    {
                        pos_drone_temp = Eigen::Vector3d(pos_drone_temp[0] + 0.05, 0.0, 0.0);
                ...
                if(flag_wall == 3)
                {
                    wall_right = Eigen::Vector3d(pos_drone_fcu[0] + wall_delta[0], pos_drone_fcu[1] + wall_delta[1], 0.8);
                    wall_left = Eigen::Vector3d(wall_right[0] + wall_direct[0]*LAMDA_WALL, wall_right[1] + wall_direct[1]*LAMDA_WALL, 0.8);
                    pos_drone_now = Eigen::Vector3d(pos_drone_fcu[0], pos_drone_fcu[1], 0.8);
                ...
                if(flag_wall == 4)
                {
                    ...
                    {
                        if(wall_right[1] > pos_drone_now[1]) {pos_drone_now = Eigen::Vector3d(pos_drone_now[0], pos_drone_now[1] + 0.05, 0.8);}
                        else                                 {pos_drone_now = Eigen::Vector3d(pos_drone_now[0], pos_drone_now[1] - 0.05, 0.8);}
                    ...
                    else
                    {
                        ...
                        {
                            if(wall_right[0] > pos_drone_now[0]) {pos_drone_now = Eigen::Vector3d(pos_drone_now[0] + 0.05, pos_drone_now[1], 0.8);}
                            else                                 {pos_drone_now = Eigen::Vector3d(pos_drone_now[0] - 0.05, pos_drone_now[1], 0.8);}
                    ...
                    if( (abs(pos_drone_fcu[0] - wall_right[0]) < 0.05) && (abs(pos_drone_fcu[1] - wall_right[1]) < 0.05) && (abs(pos_drone_fcu[2] - 0.8) < 0.05) )
                    {
                        pos_drone_now = Eigen::Vector3d(wall_right[0], wall_right[1], 0.8);
                        flag_wall = 5;
                    }
                }   
                ...
                if(flag_wall == 5)
                {
                   if(abs(pos_drone_now[1] - wall_left[1]) > 0.05 )
                    {pos_drone_now = Eigen::Vector3d(pos_drone_now[0] + wall_direct[0]*0.05, pos_drone_now[1] + wall_direct[1]*0.05, 0.8);}
                    else 
                    {pos_drone_now = Eigen::Vector3d(wall_left[0], wall_left[1], 0.8);}
                    Set_PosAim(pos_drone_now[0], pos_drone_now[1], pos_drone_now[2]);
                    local_pos_pub.publish(aim);
                    ...
                    if( (abs(pos_drone_fcu[0] - wall_left[0]) < 0.05) && (abs(pos_drone_fcu[1] - wall_left[1]) < 0.05) )//&& (abs(pos_drone_fcu[2] - 0.8) < 0.05) )
                    {
                        pos_drone_now = Eigen::Vector3d(wall_left[0], wall_left[1], 0.8);
                        pos_drone_temp = Eigen::Vector3d(0.0, 0.0, 0.0);
                        flag_wall = 6;
                ...
                if(flag_wall == 6)
                {
                    if(pos_drone_temp[0] < TRANS_TUNNEL) {pos_drone_temp = Eigen::Vector3d(pos_drone_temp[0] + 0.04, 0.0, 0.0);}
                    Set_PosAim(pos_drone_now[0] + pos_drone_temp[0], pos_drone_now[1], pos_drone_now[2]);
                    local_pos_pub.publish(aim);

                    if(abs(pos_drone_fcu[0] - wall_left[0] - TRANS_TUNNEL) < 0.05) {flag_wall = 7;}
                }
            ...
            {    
                ...
                if(flag_tunnel == 0)
                {
                    pos_drone_now = Eigen::Vector3d(pos_drone_fcu[0], pos_drone_fcu[1], 1.75);
                    pos_drone_temp = Eigen::Vector3d(0.0, 0.0, 0.0);
                    search_direct = 1;
                    count_tunnel = 0;
                    last_recog = ros::Time::now();
                ...
                if(flag_tunnel == 3)
                {       
                    tunnel_before = Eigen::Vector3d(tunnel_entry[0], tunnel_entry[1], 1.75);
                    tunnel_after = Eigen::Vector3d(tunnel_before[0] + LAMDA_TUNNEL, tunnel_before[1], 1.75);
                ...
                {    
                    ...
                    if( (abs(pos_drone_fcu[0] - tunnel_before[0]) < 0.05) && (abs(pos_drone_fcu[1] - tunnel_before[1]) < 0.05) && (abs(pos_drone_fcu[2] - 1.75) < 0.05) )
                    {
                        flag_tunnel = 5;
                    }
                }
                ...
                if(flag_tunnel == 5)
                {
                    ...
                    if( (abs(pos_drone_fcu[0] - tunnel_after[0]) < 0.05) && (abs(pos_drone_fcu[1] - tunnel_after[1]) < 0.05) && (abs(pos_drone_fcu[2] - 1.75) < 0.05) )
                    {
                        pos_drone_now = Eigen::Vector3d(tunnel_after[0], tunnel_after[1], 0.8);
                        pos_drone_temp = Eigen::Vector3d(0.0, 0.0, 0.0);
                        flag_tunnel = 6;
                    }
                }
                ...
                if(flag_tunnel == 6)
                {
                    if(pos_drone_temp[0] < TRANS_PLAN) 
                    {
                        pos_drone_temp = Eigen::Vector3d(pos_drone_temp[0] + 0.04, 0.0, 0.0);
                        Set_PosAim(pos_drone_now[0] + pos_drone_temp[0], pos_drone_now[1], pos_drone_now[2]);
                        local_pos_pub.publish(aim);
                    }
                    else
                    ...
                    if( (abs(pos_drone_fcu[0] - tunnel_after[0] - TRANS_PLAN) < 0.05) && ( (QuaterToYaw(orient_drone_fcu[0], orient_drone_fcu[1], orient_drone_fcu[2], orient_drone_fcu[3]) + 90.0) < 3.0) )
                    {
                        flag_tunnel = 7;
                    }
                }
            ...
            {       
                ...
                if(flag_circle == 0)
                {
                    // update current position: current x, y    fixed z = 1.5
                    pos_drone_now = Eigen::Vector3d(pos_drone_fcu[0], pos_drone_fcu[1], 1.5);
                    // reset yaw_search
                    yaw_search = 179.0;
                    // rotate right first
                    rotate_direct = 1;
                ...
                /*----------wait and check circle recognition----------*/
                if(flag_circle == 1)
                {
                    Set_PosAim(pos_drone_now[0], pos_drone_now[1], pos_drone_now[2]);
                    Set_TargetYaw(yaw_search);
		            local_target_pub.publish(target);
                    ROS_INFO("Rec: %d, stage: D, flag: %d, aim: %.3lf, %.3lf, %.3lf, %.3lf", cmdd, flag_circle, aim.pose.position.x, aim.pose.position.y, aim.pose.position.z, yaw_search);
                    ROS_INFO("Rec: %d, recog: %d, count: %d", cmdd, flag_circle_recog, count_circle);
                    
                    /*----------check ring recognition----------*/
                    if(count_circle < COUNT_CIRCLE)
                    {
                        if(flag_circle_recog == 1)
                        {
                            count_circle = count_circle + 1;
                            last_recog = ros::Time::now();
                            flag_circle_recog = 0;    flag_ring_recog = 0;
                        }
                    ...
                    /*----------COUNT_CIRCLE times recognition as successful----------*/
                    else {flag_circle = 3;}
                }
                ...
                {    
                    ...
                    Set_TargetYaw(yaw_search);
                    local_target_pub.publish(target);
                    ROS_INFO("Rec: %d, stage: D, flag: %d, %.3lf, %.3lf, %.3lf, %.3lf", cmdd, flag_circle, aim.pose.position.x, aim.pose.position.y, aim.pose.position.z, yaw_search);
                    ROS_INFO("Rec: %d, recog: %d, count: %d", cmdd, flag_circle_recog, count_circle);
                ...
                /*----------determine point before and after circle----------*/
                if(flag_circle == 3)
                {       
                    if(yaw_search >= 0.0) {yaw_yaw = yaw_search - 180.0;}
                    else                  {yaw_yaw = yaw_search + 180.0;}
                    ring_delta = Eigen::Vector3d(ring_delta[0]*cos(yaw_yaw*PI/180.0) - ring_delta[1]*sin(yaw_yaw*PI/180.0), ring_delta[1]*cos(yaw_yaw*PI/180.0) + ring_delta[0]*sin(yaw_yaw*PI/180.0), ring_delta[2]);                    
                    circle_center = pos_drone_fcu + ring_delta;
                    circle_before = Eigen::Vector3d(circle_center[0] + 0.5, circle_center[1], circle_center[2]);
                    circle_after = Eigen::Vector3d(circle_center[0] - 0.5, circle_center[1], circle_center[2]);

                    flag_circle = 4;
                }

                /*----------fly towards point before circle----------*/
                if(flag_circle == 4)
                {    
                    if(abs(QuaterToYaw(orient_drone_fcu[0], orient_drone_fcu[1], orient_drone_fcu[2], orient_drone_fcu[3]) - 178.0) > 2.0)
                    {
                        Set_PosAim(pos_drone_now[0], pos_drone_now[1], pos_drone_now[2]);
                        Set_TargetYaw(178.0);
                        local_target_pub.publish(target);
                    ...
                    ROS_INFO("Rec: %d, stage: D, flag: %d, aim: %.3lf, %.3lf, %.3lf", cmdd, flag_circle, aim.pose.position.x, aim.pose.position.y, aim.pose.position.z);
                    ROS_INFO("Rec: %d, before: %.3lf, %.3lf, %.3lf, after: %.3lf, %.3lf, %.3lf", cmdd, circle_before[0], circle_before[1], circle_before[2], circle_after[0], circle_after[1], circle_after[2]);

                    if( (abs(pos_drone_fcu[0] - circle_before[0]) < 0.05) && (abs(pos_drone_fcu[1] - circle_before[1]) < 0.05) && (abs(pos_drone_fcu[2] - circle_before[2]) < 0.05) )
                    {
                        flag_circle = 5;
                    }
                }
                ...
                if(flag_circle == 5)
                {
                    Set_PosAim(circle_after[0], circle_after[1], circle_after[2]);
                    local_pos_pub.publish(aim);
                    ROS_INFO("Rec: %d, stage: D, flag: %d, aim: %.3lf, %.3lf, %.3lf", cmdd, flag_circle, aim.pose.position.x, aim.pose.position.y, aim.pose.position.z);
                    ROS_INFO("Rec: %d, before: %.3lf, %.3lf, %.3lf, after: %.3lf, %.3lf, %.3lf", cmdd, circle_before[0], circle_before[1], circle_before[2], circle_after[0], circle_after[1], circle_after[2]);

                    if( (abs(pos_drone_fcu[0] - circle_after[0]) < 0.05) && (abs(pos_drone_fcu[1] - circle_after[1]) < 0.05) && (abs(pos_drone_fcu[2] -circle_after[2]) < 0.05) )
                    {   
                        pos_drone_now = Eigen::Vector3d(circle_after[0], circle_after[1], circle_after[2]);
                        yaw_search = 0.0;
            ...
            {       
                ...
                {       
                    ...
                    ring_before = Eigen::Vector3d(ring_center[0] + 0.5, ring_center[1], ring_center[2]);
                    ring_after = Eigen::Vector3d(ring_center[0] - 0.5, ring_center[1], ring_center[2]);
            ...
            if(flag_LAND == 1)
            {
                if(flag_tag == 0)
                {
                    pos_drone_now = Eigen::Vector3d(pos_drone_fcu[0], pos_drone_fcu[1], 1.2);         //!!!!!!!!!!!!!!!!!!!no 
                    pos_drone_temp = Eigen::Vector3d(0.0, 0.0, 0.0);
                    search_direct = 1;
                    count_tag = 0;
                    last_recog = ros::Time::now();
