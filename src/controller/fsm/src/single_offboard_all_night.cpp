...
void Set_TargetYaw(double yaw)
{
    ...
    if(abs(target.yaw - yaw) > 3.0)
    {
        if(yaw > target.yaw) {target.yaw = target.yaw + 1.5;}
        else                 {target.yaw = target.yaw - 1.5;}
    }
    else {target.yaw = yaw;}
...
double QuaterToYaw(double x, double y, double z, double w)
{
    ...
    double yaw_angle = current_yaw*180.0/PI;
    return yaw_angle;
}
...
//BUG
    ...
    nh.param("single_offboard/Mission_D_orientation" , Mission_D_orientation, -1.0);
...
//        if(cmdd!=4) local_pos_pub.publish(aim);
        ...
        if(cmdd == 7)
        {
            ...
            {    
                ...
                if(flag_wall == 5)
                {
                    pos_drone_now = Eigen::Vector3d(pos_drone_now[0] + wall_direct[0]*0.02, pos_drone_now[0] + wall_direct[0]*0.02, 0.7);
                    Set_PosAim(pos_drone_now[0], pos_drone_now[1], pos_drone_now[2]);
                    local_pos_pub.publish(aim);
                ...
                if(flag_wall == 6)
                {
                    if(pos_drone_temp[0] < 1.6) {pos_drone_temp = Eigen::Vector3d(pos_drone_temp[0] + 0.02, 0.0, 0.0);}
                    Set_PosAim(pos_drone_now[0] + pos_drone_temp[0], pos_drone_now[1], pos_drone_now[2]);
                    local_pos_pub.publish(aim);

                    if(pos_drone_temp[0] >= 1.6)
                    {
                        flag_wall = 7;
                        pos_drone_temp = Eigen::Vector3d(0.0, 0.0, 0.0);
                    }
            ...
            if(flag_C == 1)
        {
            ...
            */
            ...
            {               
                std::pair<double, double> yaw_Mission_D = calculate_yaw(last_yaw,Mission_D_orientation);
                geometry_msgs::Quaternion D_q = tf::createQuaternionMsgFromYaw(yaw_Mission_D.first);
                cout<<"============"<<endl;
                cout<<current_yaw_-Mission_D_orientation<<endl;
                cout<<current_yaw_<<endl;
                aim.pose.orientation = D_q;
                local_pos_pub.publish(aim);
                if(abs(current_yaw_-Mission_D_orientation) < PI/18)
                {
                    flag_C = 0;
                    flag_D = 1;
...
ROS_INFO("Rec: %d, %.3lf, %.3lf, %.3lf",cmdd,aim.pose.position.x,aim.pose.position.y,aim.pose.position.z);
            ...
            {       
                ...
                if( (flag_ring == 1) || (flag_ring == 6) || (flag_ring == 11) )
                {
                    ...
                    ROS_INFO("Rec: %d, recog: %d, count: %d", cmdd, flag_ring_recog, count_ring);
