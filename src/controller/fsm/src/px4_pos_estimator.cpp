...
ros::Subscriber t265_sub = nh.subscribe<nav_msgs::Odometry>("odom", 10, t265_cb);
    
    //【订阅】VINS数据
    //ros::Subscriber VINS_sub = nh.subscribe<nav_msgs::Odometry>("/VIO/odom", 10, VINS_cb);
    ros::Subscriber VINS_sub = nh.subscribe<nav_msgs::Odometry>("/vins_estimator/odometry", 10, VINS_cb);
    // 【订阅】超声波的数据
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Main Loop<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        if(fabs(pos_drone_t265[0]-pos_drone_fcu[0])<1 && fabs(pos_drone_t265[1]-pos_drone_fcu[2])<1 
           && fabs(pos_drone_t265[2]-pos_drone_fcu[2])<1 && fabs(Euler_t265[2] * 180/M_PI-Euler_fcu[2] * 180/M_PI)<1)
        { READY.ready=1;
          ready.publish(READY);
        }
        rate.sleep();
...
void printf_info()
{
    ...
    if(flag_use_laser_or_vicon == 0)
    {
        ...
        cout << "Pos_Optitrack [X Y Z] : " << pos_drone_mocap[0] << " [ m ] "<< pos_drone_mocap[1] <<" [ m ] "<< pos_drone_mocap[2] <<" [ m ] "<<endl;
        cout << "Euler_Optitrack [Yaw] : " << Euler_mocap[2] * 180/M_PI<<" [deg]  "<<endl;
    }else if(flag_use_laser_or_vicon == 1)
    {
