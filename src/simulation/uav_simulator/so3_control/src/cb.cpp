...
{
  if(BTraj.size() != 0){
  if(seq_iter <= (*(BTraj.end()-1)).seq_)
        {
            bs_traj BT_ptr = BTraj[seq_iter++];
            // CAL_YAW
...
void newbspline_subCallback(const bspline_race::BsplineTrajConstPtr &msg)
{
  ...
  else
  {
    ...
    int delta_seq = new_traj_start_seq - seq_iter;
    if(delta_seq<0)// 如果当前发过来的轨迹太旧
    {
      ROS_WARN("The delay is too large, check the parameter Settings.");
      // // FIXME 需要从新轨迹上的对应时刻取点
      // if( seq_iter < (msg->current_seq + msg->position.size()))// 如果当前执行的时刻小于
      // {
    ...
    else// 终于没事了
    {
      for (size_t i = 0; i < msg->position.size(); i++)
      {
        BTraj[i + new_traj_start_seq].pos_<<msg->position[i].pose.position.x, 
                                            msg->position[i].pose.position.y, 
                                            msg->position[i].pose.position.z;
        BTraj[i + new_traj_start_seq].vel_<<msg->position[i].pose.position.x, 
                                            msg->position[i].pose.position.y, 
                                            msg->position[i].pose.position.z;
        BTraj[i + new_traj_start_seq].acc_<<msg->position[i].pose.position.x, 
                                            msg->position[i].pose.position.y, 
                                            msg->position[i].pose.position.z;
        BTraj[i + new_traj_start_seq].seq_ = i + new_traj_start_seq;
      }
    }
  }
